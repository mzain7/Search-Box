...
const Tag: React.FC<TagProps> = ({ label, isActive = false, onClick }) => {
  return (
    <button
      className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-medium transition-colors group hover:bg-primaryhover hover:text-white ${
        isActive ? "bg-primary text-white" : "bg-gray text-primary"
      }`}
      onClick={onClick}
    >
      <span>
        ...
        >
          <path
            ...
            strokeWidth="1.5"
          />
          <circle cx="8.83789" cy="8.47278" r="0.625" strokeWidth="1.5" />
        </svg>
      </span>
      {label}
    </button>
